...
# Source: openinference.semconv.trace.SpanAttributes
# ---------------------------------------------------------------------------
...
_OI_LLM_INVOCATION_PARAMETERS = "llm.invocation_parameters"
...
_OI_LLM_TOKEN_COUNT_CACHE_READ = "llm.token_count.prompt_details.cache_read"
_OI_LLM_TOOLS = "llm.tools"
_OI_AGENT_NAME = "agent.name"

# ---------------------------------------------------------------------------
# OpenLLMetry / Traceloop attribute keys
# Source: opentelemetry.semconv_ai.SpanAttributes + gen_ai_attributes
# ---------------------------------------------------------------------------
_TL_SPAN_KIND = "traceloop.span.kind"
_TL_ENTITY_NAME = "traceloop.entity.name"
_TL_ENTITY_INPUT = "traceloop.entity.input"
_TL_ENTITY_OUTPUT = "traceloop.entity.output"
_TL_ENTITY_PATH = "traceloop.entity.path"
_RESPAN_LOG_TYPE = "respan.entity.log_type"
...
class OpenInferenceTranslator(SpanProcessor):
    ...
    def on_end(self, span: ReadableSpan) -> None:
        ...
        attrs.setdefault(_TL_SPAN_KIND, traceloop_kind)
        attrs.setdefault(_RESPAN_LOG_TYPE, _OI_KIND_TO_LOG_TYPE.get(oi_kind_upper, "task"))

        # --- Entity name ---
        entity_name = attrs.get(_OI_AGENT_NAME) or span.name
        attrs.setdefault(_TL_ENTITY_NAME, entity_name)

        # --- Entity path (empty = root candidate) ---
        attrs.setdefault(_TL_ENTITY_PATH, "")

        # --- Input / output (reverse of Arize _map_generic_span) ---
        input_val = attrs.get(_OI_INPUT_VALUE)
        if input_val is not None:
            attrs.setdefault(_TL_ENTITY_INPUT, _safe_json_str(input_val))

        output_val = attrs.get(_OI_OUTPUT_VALUE)
        if output_val is not None:
            attrs.setdefault(_TL_ENTITY_OUTPUT, _safe_json_str(output_val))
        ...
        system = attrs.get(_OI_LLM_SYSTEM)
        if system:
            attrs.setdefault(GEN_AI_SYSTEM, str(system).lower())

        provider = attrs.get(_OI_LLM_PROVIDER)
        if provider:
        ...
        total_tokens = attrs.get(_OI_LLM_TOKEN_COUNT_TOTAL)
        if total_tokens is not None:
            attrs.setdefault(_TL_USAGE_TOTAL_TOKENS, total_tokens)

        cache_read = attrs.get(_OI_LLM_TOKEN_COUNT_CACHE_READ)
    ...
    def _translate_llm(self, attrs: Dict[str, Any]) -> None:
        ...
        inv_params_raw = attrs.get(_OI_LLM_INVOCATION_PARAMETERS)
        if inv_params_raw:
        ...
        tools_raw = attrs.get(_OI_LLM_TOOLS)
        if tools_raw:
